...
class CarveThread extends Thread {
	...
	private boolean _slow;
        private Tile exitTile = null; // Tile wich represents the exit
	...
	private void pierceExit() {
			...
			if (tile.isExit(_model.getWidth(), _model.getHeight())) {
				if (tile.getNeighbors(_model.getTiles(), ETileType.Empty)
					...
					tile.setType(ETileType.Empty);
                                        exitTile = tile; //ROH
                                        _model.findLongestPathFrom(exitTile);
					break;
				}
			}
		}
        ...
        /**
         * ROH : tile wich represents the entrance
         * The maximaum path is calculated
         */
        private void entranceTile() {
