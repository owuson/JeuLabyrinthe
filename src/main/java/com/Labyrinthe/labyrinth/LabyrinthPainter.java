...
/**
 * Paintscom.Labyrinthe. a labyrinth model.
 * 
...
public class LabyrinthPainter extends JPanel {
	...
	private boolean _fancyGraphcis = true;
        ...
        //private Tile lastTilePosition = null; // tile which is the last red tile
	...
	public void searchPath(Point point) {
        ...
        public void searchPath2(Point point) {
            if (_labyrinth.isGenerating() || _labyrinth.isGagne() == true) {
                    return;
            }
            List<Tile> tiles = _labyrinth.getTiles();
            Tile startTile = null;
            for (Tile tile : tiles)
            {
                if (tile.getType() != Tile.ETileType.Empty)
                    continue; // c'est une case avec un mur
                int x = tile.getCoordinate().getX() * _tileSize;
                int y = tile.getCoordinate().getY() * _tileSize;

                Rectangle rect = new Rectangle(x, y, _tileSize, _tileSize);

                if (rect.contains(point)) { // la souris est dans cette case
                    // est ce une case adjacente du dernier endroit ?
                    int lastX = _labyrinth.getLastTilePosition().getCoordinate().getX();
                    int currentX = tile.getCoordinate().getX();
                    int lastY = _labyrinth.getLastTilePosition().getCoordinate().getY();
                    int currentY = tile.getCoordinate().getY();

                    if (tile.isNeighbor(_labyrinth.getLastTilePosition())){
                        _labyrinth.getLastTilePosition().setPath(true); // pastille jaune
                        tile.setStart(true);
                        _labyrinth.setLastTilePosition(tile);       // pastille rouge
                    }
                    if (_labyrinth.getLastTilePosition().isExit(_labyrinth.getWidth(), _labyrinth.getHeight()))
                    {   // GAGNE !
	...
	private void paintEmpty(Graphics2D g, int x, int y, Tile tile) {
		if (tile.isStart()) {
			...
			g.fillOval(x + 1, y + 1, _tileSize - 2, _tileSize - 2);
                        System.out.println("Rouge id=" + tile.id + "," + tile.getCoordinate().getX() + "," + tile.getCoordinate().getY());
                                
		} else if (tile.isPath()) {
			g.setColor(PATH_TILE_BACKGROUND_COLOR);
			g.fillOval(x + 1, y + 1, _tileSize - 2, _tileSize - 2);
                        System.out.println("Jaune id=" + tile.id + "," + tile.getCoordinate().getX() + "," + tile.getCoordinate().getY());
		}
	}
